...
```{r}
phylogenies <- wiritttes::get_posteriors(read_file(filename))[[1]][[1]]$trees
...
```{r}
true_gamma_statistics <- collect_file_species_tree_gammas(filename = filename)
...
```{r}
true_nltt_values <- collect_file_species_tree_nltts(filename = filename, dt = dt)
