...
```{r}
wiritttes::do_simulation(filename, cache_beast_output = TRUE)
```

## Show the posterior

```{r}
posteriors <- wiritttes::get_posteriors(read_file(filename))
testit::assert(length(posteriors) == 2 * n_beast_runs)
testit::assert(RBeast::is_posterior(wiritttes::get_posterior(file = read_file(filename), sti = 1, ai = 1, pi = 1)))
testit::assert(RBeast::is_posterior(wiritttes::get_posterior(file = read_file(filename), sti = 1, ai = 1, pi = 2)))
testit::assert(RBeast::is_posterior(wiritttes::get_posterior(file = read_file(filename), sti = 2, ai = 1, pi = 1)))
testit::assert(RBeast::is_posterior(wiritttes::get_posterior(file = read_file(filename), sti = 2, ai = 1, pi = 2)))
